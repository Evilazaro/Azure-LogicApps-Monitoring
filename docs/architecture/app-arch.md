...
flowchart TB
    ...
    accDescr: High-level view of the three application units (eShop.Web.App, eShop.Orders.API, OrdersManagementLogicApp) and their relationships to Azure infrastructure services. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph infra["☁️ Azure Infrastructure"]
        direction TB
        SQL[("🗃️ Azure SQL Database")]:::warning
        SB("📤 Azure Service Bus<br/>ordersplaced topic"):::warning
        Blob[("📦 Azure Blob Storage<br/>order results")]:::warning
...
flowchart LR
    ...
    accDescr: Shows the six application collaborations between eShop.Web.App, eShop.Orders.API, OrdersManagementLogicApp, and Azure infrastructure. Synchronous REST shown as solid arrows, asynchronous messaging as dashed. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph azure["☁️ Azure Services"]
        SQL[("🗃️ Azure SQL DB")]:::warning
        SB("📤 Service Bus<br/>ordersplaced"):::warning
        Blob[("📦 Blob Storage")]:::warning
    end
...
> ⚠️ **Operational Requirement:** Dead-letter queue monitoring MUST be implemented. Failures in the Service Bus `ordersplaced` subscription will be silently dropped without DLQ subscriber and alerting.
...
> ⚠️ **Security Requirement:** No passwords or credential secrets may exist in any application configuration file or environment variable. Managed Identity is the only permitted authentication method.
...
flowchart TB
    ...
    accDescr: Eight architecture principles organized into Structural, Integration, and Operational categories each with implementation evidence and enforcement points. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph integration["🔗 Integration Principles"]
        ...
        P8("📦 P8: Shared Library<br/>Cross-Cutting"):::warning
    end
...
| Capability Area                               | Maturity Level |
| --------------------------------------------- | -------------- |
| Orders API — CRUD Endpoints                   | 4 — Managed    |
| Orders API — Service Bus Publishing           | 4 — Managed    |
| Orders API — OTel Tracing and Metrics         | 4 — Managed    |
| Orders API — Resilience (Retry + Health)      | 4 — Managed    |
| Web App — Blazor Server UI (5 pages)          | 4 — Managed    |
| Web App — Typed HTTP Client with Discovery    | 4 — Managed    |
| Web App — Session State                       | 2 — Repeatable |
| Logic App — Service Bus Triggered Processing  | 3 — Defined    |
| Logic App — Blob Archival (success + failure) | 3 — Defined    |
| API Governance — Versioning                   | 2 — Repeatable |
| Contract Testing — Consumer-Driven            | 1 — Initial    |
...
flowchart TB
    ...
    accDescr: Current state layered architecture showing all components within Orders API, Web App, and Logic App with their dependencies to Azure infrastructure. Warning node indicates in-memory session gap. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph azure["☁️ Azure Services"]
        SQL[("🗃️ Azure SQL DB<br/>99.99% SLA")]:::warning
        SB("📤 Service Bus<br/>ordersplaced topic"):::warning
        Blob[("📦 Blob Storage<br/>99.9% SLA")]:::warning
    end
    ...
    style webapp fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style ordersapi fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style logicapp fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style azure fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
```
...
flowchart LR
    ...
    accDescr: Deployment view showing four application services and their hosting environments. eShop.Web.App and eShop.Orders.API in Azure Container Apps, OrdersManagementLogicApp in Azure Logic Apps Standard, app.ServiceDefaults as shared library. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
...
flowchart LR
    ...
    accDescr: Component-level interaction flow for order placement showing Web App pages, API service client, Orders controller, service, repository, SQL, and Service Bus. Solid arrows synchronous, dashed asynchronous. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph infracomp["☁️ Azure Services"]
        SQLDB[("🗃️ Azure SQL DB")]:::warning
        SBTopic("📤 ordersplaced<br/>topic"):::warning
    end
    ...
    style infracomp fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
```
...
flowchart TB
    ...
    accDescr: Full integration map showing all application dependencies with direction, protocol, and SLA chain. Synchronous dependencies shown as solid arrows, asynchronous as dashed arrows. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph azure["☁️ Azure Platform Services"]
        SQL[("🗃️ Azure SQL<br/>99.99% SLA")]:::warning
        SB("📤 Azure Service Bus<br/>99.9% SLA"):::warning
        Blob[("📦 Azure Blob Storage<br/>99.9% SLA")]:::warning
    ...
    style appunits fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style azure fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
```
...
flowchart LR
    ...
    accDescr: End-to-end flow showing order placement through web app, API persistence and publish to Service Bus, Logic App trigger, HTTP callback to process endpoint, and diverging blob archival paths for success and failure. AZURE/FLUENT v1.1 semantic palette: core=primary application services (blue), neutral=shared library and defaults (grey), success=operational components (green), warning=Azure infrastructure and caution (amber), danger=error conditions (red). WCAG AA compliant.
    ...
    subgraph sync["🔄 Synchronous Placement Path"]
        ...
        DB[("🗃️ SQL DB<br/>save order")]:::warning
    end

    subgraph async["⚡ Asynchronous Processing Path"]
        SBT("📤 Service Bus<br/>ordersplaced topic"):::warning
    ...
    style sync fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
