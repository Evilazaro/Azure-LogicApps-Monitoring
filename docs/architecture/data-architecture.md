...
This section documents key architectural decisions (ADRs) inferred from the source code and infrastructure definitions. While no formal ADR documents were detected in the repository, several significant design decisions are evident from the implementation patterns and technology choices.

The decisions cover storage technology selection, authentication strategy, messaging architecture, and data access patterns. Each ADR captures the context observable in the code, the decision made, and the consequences of that choice.
...
#### 6.1.1 ADR-001: Entra ID-Only Authentication for SQL Server

**Context**: The system requires secure database access without managing SQL passwords or connection strings. Azure SQL Server supports both SQL authentication and Entra ID authentication.

**Decision**: Use `azureADOnlyAuthentication: true` with User-Assigned Managed Identity for all SQL Server access. SQL password authentication is completely disabled.

**Rationale**: Eliminates credential rotation burden, prevents credential leakage, aligns with Zero Trust principles, and enables centralized identity governance through Entra ID.

**Consequences**: Requires all clients to support Entra ID token-based authentication. Local development requires Azure CLI login or DefaultAzureCredential configuration. Deployment scripts cannot use SQL credentials directly.

**Source**: `infra/shared/data/main.bicep:530-575`

#### 6.1.2 ADR-002: Event-Driven Order Processing via Service Bus Topics

**Context**: Order processing requires decoupling between the API that receives orders and the workflow that processes them. Multiple consumers may need to process the same order events.

**Decision**: Use Azure Service Bus topics (pub/sub) with the `ordersplaced` topic and `orderprocessingsub` subscription, including dead-letter queue configuration.

**Rationale**: Topics enable multiple subscribers, dead-letter queues handle poison messages, and lock-based processing prevents duplicate handling. The 14-day TTL provides adequate recovery window.

**Consequences**: Introduces eventual consistency between order persistence (SQL) and downstream processing (Logic Apps). Requires monitoring of dead-letter queue depth and subscription health.

**Source**: `infra/workload/messaging/main.bicep:121-142`

#### 6.1.3 ADR-003: Repository Pattern with EF Core for Data Access

**Context**: The application needs a data access layer that separates business logic from database concerns and supports testability through dependency injection.

**Decision**: Implement `IOrderRepository` interface with EF Core-based `OrderRepository`, using separate domain models (`Order`) and persistence entities (`OrderEntity`) with explicit mappers.

**Rationale**: Interface-driven design enables mock substitution in tests, domain-entity separation prevents ORM concerns from leaking into business logic, and the mapper pattern provides explicit transformation control.

**Consequences**: Requires maintaining parallel model hierarchies (domain + entity) and mapper code. Trade-off is accepted for cleaner architecture and testability.

**Source**: `src/eShop.Orders.API/Interfaces/IOrderRepository.cs:1-77`

#### 6.1.4 ADR-004: Blob Storage Segregation by Processing Outcome

**Context**: Order processing workflows need to track outcomes (success, error, completed) for audit and operational monitoring.

**Decision**: Create three separate blob containers: `ordersprocessedsuccessfully`, `ordersprocessedwitherrors`, `ordersprocessedcompleted`. Each container stores processing results by outcome type.

**Rationale**: Segregation enables independent lifecycle management, simplifies monitoring (container-level metrics), and allows different retention policies per outcome type. The cleanup workflow separately manages the success container.

**Consequences**: Requires coordination between Logic App workflows to route to correct containers. No cross-container queries without Azure Data Explorer or similar analytics tool.

**Source**: `infra/shared/data/main.bicep:197-222`
...
This section defines the data architecture standards observed in the source code and infrastructure definitions. Standards ensure consistency, maintainability, and compliance across the data estate. These standards are enforced through code conventions, framework configurations, and infrastructure-as-code patterns rather than separate policy documents.

The primary standards cover naming conventions for database entities, schema design guidelines enforced through EF Core Fluent API, data quality rules implemented as validation attributes, and security requirements codified in Bicep modules. Adopting formal standards documentation is recommended for future governance maturity improvements.

For mature data platforms, standards are typically stored in `/docs/standards/` and enforced through automated validation in CI/CD pipelines.

### Data Naming Conventions

| Category           | Convention                   | Example                                 | Source                                               |
| ------------------ | ---------------------------- | --------------------------------------- | ---------------------------------------------------- |
| C# Entities        | PascalCase class names       | `OrderEntity`, `OrderProductEntity`     | src/eShop.Orders.API/data/Entities/OrderEntity.cs:16 |
| C# Properties      | PascalCase property names    | `CustomerId`, `DeliveryAddress`         | app.ServiceDefaults/CommonTypes.cs:89-90             |
| SQL Tables         | Pluralized PascalCase        | `Orders`, `OrderProducts`               | src/eShop.Orders.API/data/OrderDbContext.cs:58-59    |
| Blob Containers    | lowercase concatenated       | `ordersprocessedsuccessfully`           | infra/shared/data/main.bicep:197-222                 |
| Service Bus Topics | lowercase concatenated       | `ordersplaced`                          | infra/workload/messaging/main.bicep:121-122          |
| Azure Resources    | camelCase with unique suffix | `cleanedName + wsa + uniqueSuffix`      | infra/shared/data/main.bicep:153-155                 |
| Resource Tags      | PascalCase key names         | `Solution`, `Environment`, `CostCenter` | infra/types.bicep:32-50                              |

### Schema Design Standards

| Standard          | Implementation                                                         | Source                                            |
| ----------------- | ---------------------------------------------------------------------- | ------------------------------------------------- |
| Primary Keys      | String-based, MaxLength(100), allowing GUID or custom IDs              | src/eShop.Orders.API/data/OrderDbContext.cs:63-65 |
| Foreign Keys      | Explicit FK relationships with cascade delete                          | src/eShop.Orders.API/data/OrderDbContext.cs:84-87 |
| Decimal Precision | Precision(18, 2) for all monetary fields                               | src/eShop.Orders.API/data/OrderDbContext.cs:79-80 |
| Index Strategy    | Indexes on query filter columns (CustomerId, Date, OrderId, ProductId) | src/eShop.Orders.API/data/OrderDbContext.cs:90-91 |
| Required Fields   | IsRequired() Fluent API + [Required] annotations                       | src/eShop.Orders.API/data/OrderDbContext.cs:66-80 |
| String Lengths    | MaxLength constraints on all string properties                         | src/eShop.Orders.API/data/OrderDbContext.cs:63-77 |

### Data Quality Standards

| Standard             | Implementation                                                              | Enforcement Layer        | Source                                                      |
| -------------------- | --------------------------------------------------------------------------- | ------------------------ | ----------------------------------------------------------- |
| Input Validation     | [Required], [StringLength], [Range], [MinLength] annotations                | API model binding        | app.ServiceDefaults/CommonTypes.cs:75-155                   |
| Database Constraints | MaxLength, Precision, IsRequired via Fluent API                             | EF Core / SQL DDL        | src/eShop.Orders.API/data/OrderDbContext.cs:50-129          |
| Message Retry        | maxDeliveryCount 10, lockDuration 5 min                                     | Service Bus subscription | infra/workload/messaging/main.bicep:130-131                 |
| Dead-Letter Handling | deadLetteringOnMessageExpiration, deadLetteringOnFilterEvaluationExceptions | Service Bus subscription | infra/workload/messaging/main.bicep:136-137                 |
| Idempotency          | Duplicate key detection via DbUpdateException (error 2627)                  | Repository layer         | src/eShop.Orders.API/Repositories/OrderRepository.cs:50-100 |
...
Formal data ownership assignments were not detected in source files. Based on architectural patterns observed:

| Data Domain              | Inferred Owner               | Justification                                                           | Source                                              |
| ------------------------ | ---------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------- |
| Order Transactional Data | Orders API service team      | Owns OrderEntity, OrderRepository, OrderService, OrderDbContext         | src/eShop.Orders.API/Services/OrderService.cs:1-606 |
| Order Event Data         | Platform/Infrastructure team | Owns Service Bus topic/subscription definitions and Logic App workflows | infra/workload/messaging/main.bicep:121-142         |
| Audit Trail Data         | Operations team              | Blob containers for processing outcomes, cleanup workflows              | infra/shared/data/main.bicep:197-222                |
| Observability Data       | Platform/Infrastructure team | Application Insights, Log Analytics, diagnostic settings                | infra/shared/main.bicep:\*                          |

**Recommendation**: Formalize data ownership using a RACI matrix and assign data stewards per domain.

### Access Control Model

| Resource                  | Authentication                                         | Authorization                                             | Network Control                              | Source                                                                    |
| ------------------------- | ------------------------------------------------------ | --------------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------- |
| Azure SQL Database        | Entra ID-only (azureADOnlyAuthentication)              | Entra admin via managed identity                          | Private Endpoint + DNS Zone                  | infra/shared/data/main.bicep:530-590                                      |
| Azure Storage Account     | Managed Identity + Shared Key (Logic Apps requirement) | RBAC (inferred)                                           | Private Endpoints (Blob, File, Table, Queue) | infra/shared/data/main.bicep:153-179                                      |
| Azure Service Bus         | DefaultAzureCredential (Managed Identity)              | RBAC (inferred)                                           | Public (Standard tier limitation)            | infra/workload/messaging/main.bicep:96-115                                |
| Logic App API Connections | ManagedServiceIdentity                                 | Audience-scoped (servicebus.azure.net, storage.azure.com) | Logic App integrated VNet (inferred)         | workflows/OrdersManagement/OrdersManagementLogicApp/connections.json:1-65 |

### Audit & Compliance

| Audit Control          | Implementation                                                                                                 | Scope                                      | Source                                                           |
| ---------------------- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------- |
| Resource Diagnostics   | Diagnostic settings with allLogsSettings + allMetricsSettings forwarded to Log Analytics and Storage Account   | SQL Database, Storage Account, Service Bus | infra/shared/data/main.bicep:230-240                             |
| Resource Tagging       | Mandatory 7-field tagging (Solution, Environment, CostCenter, Owner, BusinessUnit, DeploymentDate, Repository) | All Azure resources                        | infra/types.bicep:1-50                                           |
| Schema Version Control | EF Core Code-First Migrations with timestamp-based naming                                                      | SQL Database schema                        | src/eShop.Orders.API/Migrations/20251227014858_OrderDbV1.cs:1-87 |
| Distributed Tracing    | OpenTelemetry with trace context propagation across HTTP and Service Bus                                       | Orders API, Web App, Service Bus           | app.ServiceDefaults/Extensions.cs:\*                             |
| Processing Audit Trail | Blob containers segregated by processing outcome (success/error/completed)                                     | Order processing workflow                  | infra/shared/data/main.bicep:197-222                             |
