...
![CI](https://img.shields.io/github/actions/workflow/status/Evilazaro/Azure-LogicApps-Monitoring/ci-dotnet.yml?branch=main&label=CI)
...
1. **preprovision** — Validates prerequisites, builds solution, runs tests
2. **provision** — Deploys Bicep infrastructure (VNet, SQL, Service Bus, Logic Apps, etc.)
3. **postprovision** — Configures secrets, generates test data
4. **predeploy** — Deploys Logic Apps workflow definitions
5. **deploy** — Deploys .NET applications to Azure Container Apps
...
│   │   └── Components/
│   │       ├── Pages/                # Razor pages (Home, PlaceOrder, ListOrders, etc.)
...
| [Developer Hooks](docs/hooks/README.md) | Automation scripts guide (1100+ lines) |
